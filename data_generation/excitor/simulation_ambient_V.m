...
%% Run TD with pre-set amebint input on exciters

simu_type = 'ambn';
mac_fault = 0; % disabled for ambient simulation
...
dfile = 'data16m_ambient_v.m'; % pss+damping
...
%     simu_flag = s_simu64_ambient(pathname,dfile,dt_const,T_tot);
    simu_flag = s_simu64_exciter(pathname,dfile,mac_fault,dt_const,T_tot,simu_type);
else
    simu_flag = 1;
end
...
result_path = '/Users/shaohui/Documents/MATLAB/PST/68_bus_tests_V/simulation_result/';
if simu_flag == 1
    if load_all_data == 1
        filename = strcat(result_path,'impulse_td_exciter_ambient_',num2str(T_tot),'s.mat');
...
%     freq_resp = ambient_frequency_response(freq_data',n_gen,dt); % original
    freq_resp = ambient_frequency_response_para(freq_data',n_gen,dt,ipt_grp,truncation_s,filter_switch); % original 
%     freq_resp = ambient_frequency_response(freq_data(:,1:12000*15)',n_gen,dt); % 03/09/23, try 1/2min only
    ...
    filename = strcat(result_path,'ambient_1200s_inferred_freq_resp_exc.mat'); % saved on 01/30/23
    load(filename)
end
...
%         filename = strcat(result_path,'impulse_td_fault',num2str(mac_fault),'.mat');
        filename = strcat(result_path,'impz_td_0516/impulse_td_exciter_fault',num2str(mac_fault),'_',num2str(T_tot),'s.mat');
...
%% Plot voltage vals impz (05/19/23)
val_temp = bus_v_impz;
figure
for idx = 1 : n_gen0
    nexttile
    n_temp = size(val_temp{1},1);
    for i = 53 : n_temp
        if i-52 == idx
%             plot(t_impz{1},val_temp{idx}(i,:)-val_temp{idx}(i,1),'LineWidth',2)
%             temp = val_temp{idx}(i,:)-val_temp{idx}(i,1);
            temp = abs(val_temp{idx}(i,:))-abs(val_temp{idx}(i,1));
            temp = (temp(2:end)-temp(1:end-1))./dt_const;
            plot(t_impz{1}(2:end),temp./abs(max(temp)),'LineWidth',3)
            hold on
        else
%             plot(t_impz{1},val_temp{idx}(i,:)-val_temp{idx}(i,1),'LineWidth',0.75)
%             temp = val_temp{idx}(i,:)-val_temp{idx}(i,1);
            temp = abs(val_temp{idx}(i,:))-abs(val_temp{idx}(i,1));
            temp = (temp(2:end)-temp(1:end-1))./dt_const;
            plot(t_impz{1}(2:end),temp./abs(max(temp)),'LineWidth',0.5)
            hold on
        end
    end
    xlabel('Time [s]');
    ylabel('deviation');
    title(strcat('Input: exc',num2str(idx)))
end
sgt = sgtitle('|V|'' impz exc perturb');
...
sgt = sgtitle(strcat('Gen Frequency response: input exc',num2str(idx)));
sgt.FontSize = 32;
set(fig1,'Position',[10 10 1500 1200])
% saveas(gcf,strcat('RotorFreqResp_exc',num2str(idx),'.png'))
