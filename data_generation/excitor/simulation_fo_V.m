...
% FO_input = [0.57 0 0.005 0];% frequency, phase, amplitude no mode, location (0.57,0.93)
FO_input = [0.714 0 0.005 0];% frequency, phase, amplitude no mode, location 

dfile = 'data16m_ambient_v.m'; % pss with damping
...
if td_switch == 1
    for i = 1 : n_gen0
        ...
        FO_input(4) = mac_fault;
        % Run TD simulation
%         simu_flag(i) = s_simu64_fo(pathname,dfile,mac_fault,FO_input,dt_const,T_tot,ambient_input);
        simu_flag(i) = s_simu64_exciter(pathname,dfile,FO_input,dt_const,T_tot,simu_type);
    end
else
