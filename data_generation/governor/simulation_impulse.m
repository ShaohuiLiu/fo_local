...
%% Run TD for impulse on all generators
...
% dfile = 'data16m.m';
% dfile = 'data16m_ambient.m'; % additional damping added simulated on 03/08/23 
...
if td_switch == 1
    for i = 1 : n_gen0
        ...
        % Run TD simulation
        simu_flag(i) = s_simu64_impulse(pathname,dfile,mac_fault,dt_const,T_tot); % run 15s
    end
else
...
for idx = 1 : n_gen0
    mac_fault = idx;
    if simu_flag(idx) == 1
        filename = strcat('impulse_td_fault',num2str(mac_fault),'_',num2str(T_tot),'s.mat');
